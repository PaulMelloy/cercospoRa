...
```{r}
cclosure_estimates <- 
  data.table(Variable = c("Intercept","UAS","Inoculated Area","UAS : Inoculated"),
...
As expected fungicide treated areas lead to later epidemic onset, 
...
```{r}
eeeo_estimates <- 
  data.table(Variable = c("Intercept","Sentinal-2","UAS","Observed : Inoculated Area","S2 : Inoculated","UAS : Inoculated"),
             round(summary(mod5_eo)$p.table,3))[,c(1,2,3,5)] |>
   flextable() %>%
   align(align = "center", part = "all") %>%
   fontsize(size = 8, part = "body") %>%
   fontsize(size = 10, part = "header") %>%
