...
```{r eo_mod2}
mod2_eo <- gam(das ~ s(x,y, k = 11) + source + inoculum + fungicide,
          data = eeeo,
          method = "REML")
plot(mod2_eo)
```
...
```{r eo_model_data3}
mod3_eo <- gam(das ~ s(x,y, k = 11) + source * inoculum + fungicide,
            data = eeeo,
            method = "REML")
summary(mod3_eo)
```
...
```{r eo_model_data4}
mod4_eo <- gam(das ~ s(x,y, k = 11) + source * inoculum * fungicide,
            data = eeeo)
summary(mod4_eo)
```
...
```{r eo_model_data5}
mod5_eo <- gam(das ~ s(x,y) + source * inoculum,
            data = eeeo)
summary(mod5)
```
#### Intercept  
...
and averages around `r round(unname(mod5_eo$coefficients["(Intercept)"]),2)` days
after sowing.  
  
#### Remote sensing variables  
Sentinel 2 estimated epidemic onset an average of `r round(unname(mod5_eo$coefficients["sources2"]),2)`
days earlier than observed EO.  

UAS estimated epidemic onset an average of `r round(unname(mod5_eo$coefficients["sourceuav"]),2)`
days earlier than observed EO.  

#### Inoculation effect

Observed EO dates within inoculated areas were on average `r round(unname(mod5_eo$coefficients["inoculumino"]),2)`
...
`r round(unname(mod5_eo$coefficients["sources2:inoculumino"]),2)` and 
`r round(unname(mod5_eo$coefficients["sourceuav:inoculumino"]),2)` days later than
