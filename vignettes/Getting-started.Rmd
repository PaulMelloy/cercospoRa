...
### Real example of field trial data  

This weather was contributed by Facundo and is from a field trial.  

```{r}
# read in excel formated file
library(readxl)
wethr <- read_xlsx(path = "../inst/Wetterdaten_Reinshof_2022.xlsx")

# format to data.table
setDT(wethr)

# Add key data needed for format weather
# format the time
wethr[,Time := as.POSIXct(paste0(Datum, " ",Stunde,":00"),tz = "UTC")]

# nominate gps location, 
# while not needed in cercosporaR some plant disease models will use location to 
# decide the closest weather station to pull weather from
wethr[, c("lon","lat") := list(9.916,51.41866)]
# weather is hourly we need to state a standard deviation of weather direction
wethr[, wd_std := 20]

# remove all data after September as it contains missing data
wethr <- wethr[Datum < as.POSIXct("2022-10-01")]

# set NA wind speed values to zero
wethr[is.na(WG200),WG200 := 0]

# set NA wind direction values to 20 degrees. Wind is not important for this model
wethr[is.na(WR200),WR200 := 20]

weathr <- format_weather(wethr,
                         POSIXct_time = "Time",
                         time_zone = "UTC",
                         temp = "T200",
                         rain = "N100",
...
```{r}
divalues <- calc_DIV(dat = weathr[times > as.POSIXct("2022-07-01")])
